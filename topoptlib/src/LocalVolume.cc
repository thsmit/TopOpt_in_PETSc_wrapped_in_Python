...
LocalVolume::LocalVolume(DM da_nodes, Vec x, Vec xPassive, DataObj data){
...
PetscErrorCode LocalVolume::Constraint(Vec x, PetscScalar *gx, Vec dx){
	PetscErrorCode ierr;

	// LocalVolume the design variables or copy to xPhys
	// ONLY STANDARD NEIGHBORHOOD
        ierr = MatMult(H,x,xVol); CHKERRQ(ierr);
