...
def mainn(itr):
	...
	for i in range(nDom):
		...
		rawP += str(fin.read(3*4*nPointsT[i]))
	...
	for i in range(nDom):
		...
		rawP += str(fin.read(8*8*nCellsT[i]))
		...
		if int(dataset)==int(itr):	
			...
			for i in range(nDom):
				for j in range(nPFields[i]):
					...
					# Multiply by 4 == length of float32
					lrawPFields[j] += str(fin.read(4*nPointsT[i]))
				...
				for j in range(nCFields[i]):
					...
					# Multiply by 4 == length of float32
					lrawCFields[j] += str(fin.read(4*nCellsT[i]))
